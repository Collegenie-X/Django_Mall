...
class UserViewSet(viewsets.GenericViewSet):
    ...
    def signup(self, request):
        serializer = RegistrationSerializer(data=request.data)  ## email / password 
    ...
    def login(self, request):
        ...
        if not user.is_email_verified:
            return Response(
                {"detail": "Email is not verified. Please verify your email."},
        ...
        response_data = {"token": token_data, "user": user_serializer.data}
    ...
    def logout(self, request):
        ...
        auth_header = request.headers.get("Authorization")  ## Berea token
        if not auth_header:
            return Response(
...
class VerifyTokenView(APIView):
    ...
    ]  # 인증되지 않은 사용자도 이 엔드포인트에 접근 가능하게 설정
    ...
    def post(self, request):
        ...
        try:
            ...
            return Response(
                {"is_valid": True, "user": serializer.data}, status=status.HTTP_200_OK
            )
            ...
            return Response(
                {"is_valid": False, "error": "User does not exist."},
                status=status.HTTP_400_BAD_REQUEST,
            )
        except Exception as e:
            return Response(
